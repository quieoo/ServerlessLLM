...
/home/sdu/.conda/envs/sllm-worker/bin/python \
  ...
  --replacement-policy lru --routing-policy cache-bytes \
  --output docs/tensor-level-sim/target-trace-max-lru-r500.json
...
/home/sdu/.conda/envs/sllm-worker/bin/python \
  ...
  --mckp-lookahead-discount 0.9 \
  --mckp-transition-weight 0.1 \
  --output docs/tensor-level-sim/target-trace-max-mckp-r500.json
```
...
```text
docs/tensor-level-sim/target-trace-max-ttft-cdf.svg
```
...
for LAYOUT in segment tensor-page compact-page; do
  ...
  /home/sdu/.conda/envs/sllm-worker/bin/python \
    ...
    --mckp-transition-weight 0.1 \
    --output \
      "docs/tensor-level-sim/backend-comparison-max-${LAYOUT}.json"
done

python3 tools/layerpipe/plot_prefix_mckp_ttft_cdf.py \
  --series docs/tensor-level-sim/backend-comparison-max-segment.json Segment \
  --series docs/tensor-level-sim/backend-comparison-max-tensor-page.json Page \
  --series docs/tensor-level-sim/backend-comparison-max-compact-page.json \
    Page-Compact \
  --title "Prefix-MCKP backend comparison: max" \
  --output docs/tensor-level-sim/backend-comparison-max-ttft-cdf.svg
```
...
for LAYOUT in segment tensor-page compact-page; do
  ...
  /home/sdu/.conda/envs/sllm-worker/bin/python \
    ...
    --mckp-transition-weight 0.1 \
    --output \
      "docs/tensor-level-sim/backend-comparison-high-${LAYOUT}.json"
done

python3 tools/layerpipe/plot_prefix_mckp_ttft_cdf.py \
  --series docs/tensor-level-sim/backend-comparison-high-segment.json Segment \
  --series docs/tensor-level-sim/backend-comparison-high-tensor-page.json Page \
  --series docs/tensor-level-sim/backend-comparison-high-compact-page.json \
    Page-Compact \
  --title "Prefix-MCKP backend comparison: high" \
  --output docs/tensor-level-sim/backend-comparison-high-ttft-cdf.svg
```
...
for LAYOUT in segment tensor-page compact-page; do
  ...
  /home/sdu/.conda/envs/sllm-worker/bin/python \
    ...
    --mckp-transition-weight 0.1 \
    --output \
      "docs/tensor-level-sim/backend-comparison-medium-${LAYOUT}.json"
done

python3 tools/layerpipe/plot_prefix_mckp_ttft_cdf.py \
  --series docs/tensor-level-sim/backend-comparison-medium-segment.json Segment \
  --series docs/tensor-level-sim/backend-comparison-medium-tensor-page.json Page \
  --series docs/tensor-level-sim/backend-comparison-medium-compact-page.json \
    Page-Compact \
  --title "Prefix-MCKP backend comparison: medium" \
  --output docs/tensor-level-sim/backend-comparison-medium-ttft-cdf.svg
```
...
for LAYOUT in segment tensor-page compact-page; do
  ...
  /home/sdu/.conda/envs/sllm-worker/bin/python \
    ...
    --mckp-transition-weight 0.1 \
    --output \
      "docs/tensor-level-sim/backend-comparison-low-${LAYOUT}.json"
done

python3 tools/layerpipe/plot_prefix_mckp_ttft_cdf.py \
  --series docs/tensor-level-sim/backend-comparison-low-segment.json Segment \
  --series docs/tensor-level-sim/backend-comparison-low-tensor-page.json Page \
  --series docs/tensor-level-sim/backend-comparison-low-compact-page.json \
    Page-Compact \
  --title "Prefix-MCKP backend comparison: low" \
  --output docs/tensor-level-sim/backend-comparison-low-ttft-cdf.svg
```
...
for LAYOUT in segment tensor-page compact-page; do
  ...
  /home/sdu/.conda/envs/sllm-worker/bin/python \
    ...
    --mckp-transition-weight 0.1 \
    --output \
      "docs/tensor-level-sim/backend-comparison-small-${LAYOUT}.json"
done

python3 tools/layerpipe/plot_prefix_mckp_ttft_cdf.py \
  --series docs/tensor-level-sim/backend-comparison-small-segment.json Segment \
  --series docs/tensor-level-sim/backend-comparison-small-tensor-page.json Page \
  --series docs/tensor-level-sim/backend-comparison-small-compact-page.json \
    Page-Compact \
  --title "Prefix-MCKP backend comparison: small" \
  --output docs/tensor-level-sim/backend-comparison-small-ttft-cdf.svg
```
...
for LAYOUT in segment tensor-page compact-page; do
  ...
  /home/sdu/.conda/envs/sllm-worker/bin/python \
    ...
    --mckp-transition-weight 0.1 \
    --output \
      "docs/tensor-level-sim/backend-comparison-default-${LAYOUT}.json"
done

python3 tools/layerpipe/plot_prefix_mckp_ttft_cdf.py \
  --series docs/tensor-level-sim/backend-comparison-default-segment.json \
    Segment \
  --series docs/tensor-level-sim/backend-comparison-default-tensor-page.json \
    Page \
  --series docs/tensor-level-sim/backend-comparison-default-compact-page.json \
    Page-Compact \
  --title "Prefix-MCKP backend comparison: default ServeGen" \
  --output docs/tensor-level-sim/backend-comparison-default-ttft-cdf.svg
```
...
- [最大档 CDF](../docs/tensor-level-sim/backend-comparison-max-ttft-cdf.svg)
- [高档 CDF](../docs/tensor-level-sim/backend-comparison-high-ttft-cdf.svg)
- [中档 CDF](../docs/tensor-level-sim/backend-comparison-medium-ttft-cdf.svg)
- [低档 CDF](../docs/tensor-level-sim/backend-comparison-low-ttft-cdf.svg)
- [小档 CDF](../docs/tensor-level-sim/backend-comparison-small-ttft-cdf.svg)
- [默认 trace CDF](../docs/tensor-level-sim/backend-comparison-default-ttft-cdf.svg)
...
python3 tools/layerpipe/test_tensor_sim_mckp.py
python3 tools/layerpipe/test_layerweave_tensor_pipeline_sim.py
...
```bash
python3 tools/layerpipe/plot_prefix_mckp_ttft_cdf.py \
  --baseline docs/tensor-level-sim/target-trace-max-lru-r500.json \
  --mckp docs/tensor-level-sim/target-trace-max-mckp-r500.json \
  --output docs/tensor-level-sim/target-trace-max-ttft-cdf.svg \
