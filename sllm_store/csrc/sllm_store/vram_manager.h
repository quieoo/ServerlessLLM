...
enum class RegionStatus { FREE = 0, LOADING = 1, ALLOCATED = 2 };

// 为避免 ODR 违规，将函数声明为内联函数
inline std::ostream& operator<<(std::ostream& os, RegionStatus status) {
    switch(status) {
        case RegionStatus::FREE: return os << "FREE";
        case RegionStatus::LOADING: return os << "LOADING";
        case RegionStatus::ALLOCATED: return os << "ALLOCATED";
        default: return os << static_cast<int>(status);
    }
}
...
public:
    ...
    std::shared_ptr<GPUMemoryRegion_v1> prev;
    std::shared_ptr<GPUMemoryRegion_v1> next;

    GPUMemoryRegion_v1() 
        : status(RegionStatus::FREE), addr(nullptr), size(0), prev(nullptr), next(nullptr) {}

    GPUMemoryRegion_v1(void* addr_, size_t size_)
        : status(RegionStatus::FREE), addr(static_cast<char*>(addr_)), size(size_), 
          prev(nullptr), next(nullptr) {}

    bool isAdjacent(const std::shared_ptr<GPUMemoryRegion_v1>& other) const {
        return (this->addr + this->size == other->addr) || 
               (other->addr + other->size == this->addr);
    }

    void merge(const std::shared_ptr<GPUMemoryRegion_v1>& other) {
    ...
    std::string toString() {
        return "[GPUMemoryRegion_v1: addr=" +
...
struct DropCostEntry {
    double cost;
    std::shared_ptr<GPUMemoryRegion_v1> region;
...
public:
    std::shared_ptr<GPUMemoryRegion_v1> memory_regions;  // 内存区域链表头
    char* gpu_base_addr;
    std::unordered_map<std::string, std::shared_ptr<GPUMemoryRegion_v1>> allocated_regions;
    ...
    GPUTensorPool_v1(int device_id_, size_t total_size, double gpu_bw, double cpu_bw)
        ...
        memory_regions = std::make_shared<GPUMemoryRegion_v1>(gpu_memory, total_size);
        // LOG(INFO)<<"GPUTensorPool_v1: device_id="<<device_id_<<", total_size="<<total_size<<", gpu_bw="<<gpu_bw<<", cpu_bw="<<cpu_bw;
    }
    ...
    ~GPUTensorPool_v1(){
        ...
        // }
        // LOG(INFO)<<"GPUTensorPool_v1: device_id="<<device_id<<", total_move_data="<<total_move_data;
    }
    ...
    void MemoryRegionView() {
        ...
        while (current) {
            double cost=0.0;
            if (current->status == RegionStatus::ALLOCATED && current->model_ref) {
                double access_prob =
                  (total_access == 0
                       ? 1.0
    ...
    void CachedModels(){
      std::unordered_map<std::string, size_t> model_cache_size;
      std::unordered_map<std::string, size_t> model_total_size;
      for(auto region=memory_regions; region!=nullptr; region=region->next){
        if(region->status == RegionStatus::ALLOCATED && region->model_ref){
          if(model_cache_size.find(region->model_ref->model_path()) == model_cache_size.end()){
            model_cache_size[region->model_ref->model_path()]=0;
          }
          model_cache_size[region->model_ref->model_path()]+=region->size;

          if(model_total_size.find(region->model_ref->model_path()) == model_total_size.end()){
            model_total_size[region->model_ref->model_path()]=region->model_ref->model_size();
          }
        }
      }
      for(auto& pair: model_cache_size){
        double cached_gbs=double(pair.second)/(1024.0*1024.0*1024.0);
        double total_gbs=0.0;
        if(model_total_size.find(pair.first) != model_total_size.end()){
          total_gbs=double(model_total_size[pair.first])/(1024.0*1024.0*1024.0);
        }

        LOG(METRIC)<<"Model: "<<pair.first<<", cached_size: "<<cached_gbs<<" / "<<total_gbs;
      }
    }
    ...
    size_t GetFreeSize(){
        ...
        auto current = memory_regions;
        while (current) {
            if (current->status == RegionStatus::FREE) free_size += current->size;
            current = current->next;
        }
        return free_size;
    }
    ...
    void monitor_frags(const std::shared_ptr<RegisteredModel> model){
      auto current=memory_regions;
      while(current){
        if(current->status != RegionStatus::FREE && current->model_ref && current->model_ref->model_path()!=model->model_path()){
          FreeRegion(current);
          break;
        }
        current = current->next;
    ...
    int GlobalDeFrag(const std::shared_ptr<RegisteredModel> model) {
      ...
      for (const auto& r : regions) {
        if (r.status==RegionStatus::ALLOCATED) {
      ...
      std::shared_ptr<GPUMemoryRegion_v1> prev = nullptr;
      std::shared_ptr<GPUMemoryRegion_v1> head = nullptr;
      ...
      // 4. 首先处理已分配区域
      for (const auto& r : regions) {
        if (r.status!=RegionStatus::ALLOCATED) continue;

        auto new_region =
            std::make_shared<GPUMemoryRegion_v1>(current_addr, r.size);
        new_region->status = RegionStatus::ALLOCATED;
      ...
      if (total_free > 0) {
        auto free_region =
            std::make_shared<GPUMemoryRegion_v1>(current_addr, total_free);
        free_region->status = RegionStatus::FREE;
        if (!head) {
    ...
    double CostFunction(const std::shared_ptr<GPUMemoryRegion_v1>& region){
    ...
    size_t GetMergeCost(){
        ...
        // 找到第一个空闲空间和最后一个空闲空间
        std::shared_ptr<GPUMemoryRegion_v1> first_free = nullptr;
        std::shared_ptr<GPUMemoryRegion_v1> last_free = nullptr;
        while (current) {
            if (current->status == RegionStatus::FREE) {
        ...
        while (current && current != last_free) {
            if (current->status == RegionStatus::ALLOCATED) {
    ...
    void NPMWeightFunc4(
        ...
        std::vector<std::shared_ptr<GPUMemoryRegion_v1>>& free_regions) {
      size_t min_request =
      ...
      for (size_t i = 0; i < to_allocates.size(); ++i) {
        ...
        for (size_t j = 0; j < free_regions.size(); ++j) {
          ...
          while (left_region && left_region->status == RegionStatus::ALLOCATED) {
            left_move_cost += left_region->size;
            left_region = left_region->prev;
          }
          while (right_region && right_region->status == RegionStatus::ALLOCATED) {
    ...
    std::vector<std::pair<size_t, std::shared_ptr<GPUMemoryRegion_v1>>>
    WeightedBipartiteAllocate(std::vector<size_t>& to_allocates) {
        
      std::vector<std::shared_ptr<GPUMemoryRegion_v1>> free_regions;
      auto current = memory_regions;
      while (current) {
        if (current->status == RegionStatus::FREE) free_regions.push_back(current);
        current = current->next;
      }
    ...
    //   }
      ...
      std::vector<std::pair<size_t, std::shared_ptr<GPUMemoryRegion_v1>>> matches;
...
// 合并后的空闲区域指针，如果合并失败则返回nullptr
    std::shared_ptr<GPUMemoryRegion_v1> MergeRegions(
        std::shared_ptr<GPUMemoryRegion_v1> region_start,
        std::shared_ptr<GPUMemoryRegion_v1> region_end, bool is_do_move = 1) {
        size_t move_cost=0;
        // 检查输入的起始和结束区域指针是否有效
        ...
        std::vector<std::shared_ptr<GPUMemoryRegion_v1>> allocated_regions_tmp;
        for (auto cur = region_start; cur && cur != next_after; cur = cur->next) {
            if (cur->status == RegionStatus::ALLOCATED) {
            allocated_regions_tmp.push_back(cur);
            } else if (cur->status == RegionStatus::LOADING) {
            LOG(ERROR) << "MergeRegions: CANNOT MERGE RegionStatus::LOADING REGION";
            return nullptr;
            } else {
        ...
        if (free_region_size == 0) {
            LOG(ERROR) << "MergeRegions: NO RegionStatus::FREE REGION TO MERGE";
            return nullptr;
        }
        ...
        // 创建新空闲区域
        std::shared_ptr<GPUMemoryRegion_v1> free_region =
            std::make_shared<GPUMemoryRegion_v1>(current_addr, free_region_size);
    ...
    // 贪心合并策略
    std::shared_ptr<GPUMemoryRegion_v1> GreedyMerge(size_t n, bool merge=1) {
        // 先检查是否有足够大的空闲区域
        auto current = memory_regions;
        size_t total_free = 0;
        while (current) {
            if (current->status == RegionStatus::FREE && current->size >= n) return current;
            if (current->status == RegionStatus::FREE) total_free += current->size;
            current = current->next;
        }

        if(total_free < n){
            LOG(ERROR)<<"GreedyMerge: NO ENOUGH RegionStatus::FREE REGION";
            MemoryRegionView();
            return nullptr;
        }
        ...
        std::shared_ptr<GPUMemoryRegion_v1> best_s, best_e;

        current = memory_regions;
        while (current) {
            if (current->status != RegionStatus::FREE) { current = current->next; continue; }
            ...
            while (e) {
                if (e->status == RegionStatus::LOADING) break; // s和e之间不能有正在加载的区域
                if (e->status == RegionStatus::FREE) free_sum += e->size;
    ...
    void FreeRegion(std::shared_ptr<GPUMemoryRegion_v1> region, bool debug=0) {
        if(!region){
            LOG(ERROR)<<"FreeRegion: region is nullptr";
            return;
        }
        if (region->status != RegionStatus::ALLOCATED) {
        ...
        // allocated_regions.erase(region->fingerprint);
        // 合并前驱
        if (region->prev && region->prev->status == RegionStatus::FREE) {
          region->prev->merge(region);
          region = region->prev;  // 指向合并后的区域
        }
        // 合并后继
        if (region->next && region->next->status == RegionStatus::FREE) {
    ...
    std::shared_ptr<GPUMemoryRegion_v1> AllocateRegionFromList(size_t size, std::string fp) {
        for (auto region = memory_regions; region != nullptr;
             region = region->next) {
          if (region->status==RegionStatus::FREE && region->size >= size) {
            if (region->size > size) {
              auto new_region = std::make_shared<GPUMemoryRegion_v1>(
    ...
    std::shared_ptr<GPUMemoryRegion_v1> AllocateFreeRegion(
        std::shared_ptr<GPUMemoryRegion_v1> free_region, size_t allocate_size, 
        const std::string& fingerprint) {
        if (free_region->status != RegionStatus::FREE || free_region->size < allocate_size) return nullptr;
        ...
        free_region->status = RegionStatus::ALLOCATED;
...
std::pair<std::shared_ptr<GPUMemoryRegion_v1>, std::shared_ptr<GPUMemoryRegion_v1>> AllocateNewRegion(
    std::shared_ptr<GPUMemoryRegion_v1> old, size_t allocate_size,
    const std::string& fingerprint) {
  if (old->status != RegionStatus::FREE || old->size < allocate_size) {
    LOG(ERROR) << "AllocateNewRegion: old region " << old->addr
               << " status invalid (" << old->status << ")";
    return {nullptr, nullptr};
  }

  std::shared_ptr<GPUMemoryRegion_v1> new_region = nullptr;
  if (old->size > allocate_size) {
    new_region = std::make_shared<GPUMemoryRegion_v1>(
  ...
  old->status = RegionStatus::ALLOCATED;
  ...
  return {old, new_region};
    ...
    bool CheckAllocatedConsistency() {
        std::unordered_set<std::string> list_allocated;  // 链表中RegionStatus::ALLOCATED节点的指纹集合
        std::unordered_set<std::string> map_allocated;   // allocated_regions中的指纹集合
        bool is_consistent = true;

        // 遍历链表收集RegionStatus::ALLOCATED节点的指纹
        auto current = memory_regions;
        while (current) {
            if (current->status == RegionStatus::ALLOCATED) {
    ...
    int GetAvailableBlocks(std::string model_skip, size_t block_size){
      ...
      while (current) {
        ...
        }else if(current->status == RegionStatus::ALLOCATED){
    ...
    std::vector<size_t> AllocateBlocks(size_t block_size, std::string model_path, int block_number) {
      ...
      while (current && result.size() < block_number) {
        if (current->status == RegionStatus::FREE && current->size >= block_size) {
          // 计算当前空闲区域可分割的block数量
    ...
    void CleanBlocks() {
        ...
        auto current = memory_regions;
        while (current) {
          if (current->status == RegionStatus::ALLOCATED && current->fingerprint.find("KVCACHE") != std::string::npos) {
            FreeRegion(current);
          }
          current = current->next;
        }
...
void CopyMemoryLayout(GPUTensorPool_v1& other) {
    // 清空当前对象的内存区域和已分配区域
    ...
    // 复制GPU内存区域链表
    std::shared_ptr<GPUMemoryRegion_v1> other_current = other.memory_regions;
    std::shared_ptr<GPUMemoryRegion_v1> prev = nullptr;
    std::shared_ptr<GPUMemoryRegion_v1> new_head = nullptr;
    ...
    while (other_current) {
        ...
        if (new_region->status == RegionStatus::ALLOCATED) {
...
size_t MockGenerateRegions(int num_regions, size_t min_size, size_t max_size, int is_regenerate) {
  ...
  if (is_regenerate == 0) {
    ...
    if (ifs) {
      ...
      for (int i = 0; i < saved_regions; ++i) {
        auto region = std::make_shared<GPUMemoryRegion_v1>();
        ...
        if (region->status == RegionStatus::FREE) total_free += region->size;
      }
  ...
  for (int i = 0; i < num_regions; ++i) {
    ...
    if (status == RegionStatus::LOADING) {
      status = RegionStatus::ALLOCATED;  // 确保都是RegionStatus::FREE或者RegionStatus::ALLOCATED状态
    }
    ...
    // 如果是已分配状态，生成随机指纹
    if (status == RegionStatus::ALLOCATED) {
      region->fingerprint = "mock_fp_" + std::to_string(i);
      allocated_regions[region->fingerprint] = region;
    }
  }

  // 遍历链表, 合并相邻的RegionStatus::FREE区域
  auto current = memory_regions;
  bool merged;
  do {
    merged = false;
    ...
    while (current && current->next) {
      if (current->status == RegionStatus::FREE && current->next->status == RegionStatus::FREE) {
...
std::vector<size_t> MockBartiteMatching(std::vector<size_t> requests,
  ...
  return remaining;
    ...
    struct RegionAllocateGroup{
        std::shared_ptr<GPUMemoryRegion_v1> start;
        std::shared_ptr<GPUMemoryRegion_v1> end;
    ...
    */
    ...
    std::pair<RegionAllocateGroup, RegionAllocateGroup> BestSplitAllocate(
      ...
      std::vector<std::shared_ptr<GPUMemoryRegion_v1>>
      ...
      for (size_t i = 0; i < regions.size() - 1; ++i) {
        ...
        auto mid = left->next;
        while (mid != right) {
          if (mid->status == RegionStatus::ALLOCATED) {
    ...
    std::vector<RegionAllocateGroup> RecursiveSplitAllocate(
      ...
      auto current = memory_regions;
      while (current && current->status != RegionStatus::FREE) {
        current = current->next;  // 找到第一个空闲区域
      }
      initial_group.start = current;
      // current = memory_regions;
      std::shared_ptr<GPUMemoryRegion_v1> last = nullptr;
      while (current) {
        if (current->status == RegionStatus::FREE) {
          last = current;
        }
...
public:
    ...
    VRAMManager(size_t gpu_tensor_pool_size, const std::vector<int>& gpu_ids, 
        ...
        for (int gpu_id : gpu_ids) {
            gpu_tensor_pools_[gpu_id] = std::make_shared<GPUTensorPool_v1>(
                gpu_id, gpu_tensor_pool_size, gpu_bw, cpu_bw);
        }
    }

    VRAMManager(size_t reversed_vram_size, double gpu_bw, double cpu_bw, int load_strategy=4) {
    ...
    void Collect_2(){
      ...
      // 输出平均加载和合并延迟
      LOG(METRIC) << "Average Model Merge/Load Latency";
    ...
    void CachedModels(){
      ...
      // 遍历所有已分配空间，统计已缓存模型数量
      // 输出每个GPU的已缓存模型数量
      for(auto pool_it : gpu_tensor_pools_) {
        LOG(METRIC)<<"------------------POOL-"<<pool_it.first<<"------------------";
    ...
    int WithoutReuse(const std::vector<TGNeedAllocates>& tg_to_load,
                     ...
                     const std::shared_ptr<GPUTensorPool_v1> pool,
    ...
    int PartitionedBinPacking(const std::vector<TGNeedAllocates>& tg_to_load, const std::shared_ptr<RegisteredModel> model, const std::shared_ptr<GPUTensorPool_v1> pool,  std::vector<char*>& allocated_regions) {
        ...
        for (auto group : region_groups) {
          ...
          // 分配TG
          shared_ptr<GPUMemoryRegion_v1> next_free = merged_region;
          shared_ptr<GPUMemoryRegion_v1> allocated = nullptr;
          // LOG(INFO)<<"TG to load: ";
    ...
    int WeightedBipartiteMatch_GreedyMerge(const std::vector<TGNeedAllocates>& tg_to_load, const std::shared_ptr<RegisteredModel> model, const std::shared_ptr<GPUTensorPool_v1> pool,  std::vector<char*>& allocated_regions) {
        auto start_merge_time=std::chrono::high_resolution_clock::now();
    ...
    int GreedyMerge(const std::vector<TGNeedAllocates>& tg_to_load, const std::shared_ptr<RegisteredModel> model, const std::shared_ptr<GPUTensorPool_v1> pool,  std::vector<char*>& allocated_regions) {
        auto start_merge_time=std::chrono::high_resolution_clock::now();
        std::vector<TGNeedAllocates> remaining = tg_to_load;
        // 步骤5: 剩余TG按大小排序后调用GreedyMerge
        LOG(INFO)<<"Remaining TGs need to merge: "<<remaining.size();
    ...
    // 计算装载模型到指定设备需要装载多少数据（有多少数据不在显存中）
    int64_t GetToLoadSize(std::string model_path) {
      ...
      for (auto pool_it : gpu_tensor_pools_) {
        auto& pool = pool_it.second;  
        int64_t to_load_size = 0;
        for (int i = 0; i < tg_index.size(); i++) {
          auto allocated = pool->GetTensor(tg_index[i].fingerprint);
          if (!allocated) {
            to_load_size += tg_index[i].size;
          }
        }
        if(to_load_size < min_load_size){
          min_load_size = to_load_size;
        }
      }
      return min_load_size == INT64_MAX ? -1 : min_load_size;
    }
