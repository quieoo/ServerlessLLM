...
class ReuseStoreV1 {
 public:
  ReuseStoreV1(const std::string& storage_path, size_t memory_pool_size,
             int num_thread, int load_strategy);
  ~ReuseStoreV1();
  ...
  int64_t ToLoadSize(std::string model_path);
  std::vector<int64_t> ToLoadSizes(std::vector<std::string> model_paths);

  size_t GetMemPoolSize() const {return 0; }
