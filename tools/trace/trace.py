...
def preprocess_txt_trace(trace_dir):
    """Load and process txt format trace."""
    if not os.path.exists(trace_dir):
        raise RuntimeError(f"Trace file not found: {trace_dir}")
    tracelines = OrderedDict()
    print(f"Reading txt trace file: {trace_dir}")
    tic = time.time()
    with open(trace_dir, newline="") as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            function_name = row["func"]
            end_time = float(row["end_timestamp"])
            ...
            tracelines[function_name].append(start_time)

    # Sort the arrival times for each function
    for function_name, trace in tracelines.items():
        tracelines[function_name] = np.sort(np.array(trace))
    print(f"Reading takes: {time.time() - tic}s.")

    # Report stats
    num_functions = len(tracelines.keys())
    num_function_invocations = [len(trace) for trace in tracelines.values()]
    print(f"Txt trace stats: #functions: {num_functions}, total invocations: {sum(num_function_invocations)}")

    return tracelines

def load_trace(path: str) -> OrderedDict:
    if path.endswith(".txt"):
        return preprocess_txt_trace(path)
