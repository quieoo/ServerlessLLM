...
LAYERWEAVE_M4_PROFILE="${LAYERWEAVE_M4_PROFILE:-$REPO_ROOT/tools/layerpipe/results/m4-full/m4-full-estimator-report.json}"
...
RESULTS_DIR="${RESULTS_DIR:-$REPO_ROOT/tools/layerpipe/results/single_gpu}"
OUTPUT="${OUTPUT:-$RESULTS_DIR/1.2-${LOAD_MODE}-${KV_BACKEND}-${MAX_REQUESTS}-${MAX_BATCH_SIZE}-output_tokens_override_${OUTPUT_TOKENS_OVERRIDE}-result.json}"
