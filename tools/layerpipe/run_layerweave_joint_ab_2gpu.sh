...
run_one() {
  ...
  env GPU_ID="$gpu" \
    ...
    OUTPUT="$output" \
    "$@" \
    bash tools/layerpipe/run_layerpipe_real_gpu.sh > "${output%.json}.log" 2>&1
}
...
run_pair() {
  ...
  local fixed_gpu="$2"
  local joint_gpu="$3"
  local fixed="$RESULTS_DIR/joint-ab-r${round}-minimal-gpu${fixed_gpu}.json"
  local joint="$RESULTS_DIR/joint-ab-r${round}-joint-gpu${joint_gpu}.json"
  ...
  "$PYTHON" tools/layerpipe/validate_layerweave_m5_5.py \
    --fixed "$fixed" \
    --dynamic "$joint" |
    tee "$RESULTS_DIR/joint-ab-r${round}-validation.txt"
}
