#!/usr/bin/env python3
"""Plot arrival-to-first-token CDFs from online simulator request CSVs."""
...
def main():
    ...
    for system, color in zip(SYSTEMS, COLORS):
        path = args.input_dir / f"{system}-requests.csv"
        with path.open(newline="") as stream:
            values = sorted(
                float(row["ttft_ms"]) / 1000.0
                for row in csv.DictReader(stream)
                if row.get("measured", "True") == "True"
            )
        series.append((system, color, values, (
            f"{LABELS[system]} "
            f"(P50 {percentile(values, 0.5):.2f}s, "
            f"P99 {percentile(values, 0.99):.1f}s)"
        )))
    ...
    if args.html_fragment:
        ...
        svg = [
            '<div id="online-ttft-cdf" style="width:100%">',
            f'<svg xmlns="http://www.w3.org/2000/svg" role="img" '
            f'aria-label="CDF of arrival-to-first-token TTFT for five systems" '
    ...
    while 10 ** exponent <= x_max:
        ...
        exponent += 1
    for system, color, values, label in series:
        points = " ".join(
            f"{x_position(value):.2f},{y_position((index + 1) / len(values)):.2f}"
    ...
    svg.extend([
        ...
        'text-anchor="middle">Arrival-to-first-token TTFT '
        '(seconds, log scale)</text>',
        f'<text x="20" y="{top+plot_height/2:.2f}" text-anchor="middle" '
        'transform="rotate(-90 20 '
        f'{top+plot_height/2:.2f})">Cumulative fraction of requests</text>',
    ])
    ...
    for index, (_, color, _, label) in enumerate(series):
        ...
        svg.append(
            f'<text x="{legend_x+32}" y="{y+4}">{html.escape(label)}</text>')
    svg.extend(["</g>", "</svg>"])
    if args.html_fragment:
        svg.append("</div>")
