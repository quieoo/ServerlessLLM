...
# LayerWeave Real-GPU Implementation

> 本文档记录 `tools/layerpipe/` 下真实 GPU LayerPipe/LayerWeave 的设计、
> 实现和实验历史。实际路径使用 PyTorch/vLLM、CUDA stream/event 以及
> `tools/mock_allocation/` 的 VMM backend，不是性能模拟器。
...
nohup env \
    ...
    > tools/layerpipe/results/single_gpu/1.2-naive.log 2>&1 &
echo $!
```
...
nohup env \
    ...
    TRACE_TIME_SCALE=150 \
    bash tools/layerpipe/run_layerpipe_real_gpu.sh \
    > tools/layerpipe/results/single_gpu/1.2-layerpipe.log 2>&1 &
echo $!
```
关键参数：
...
mkdir -p tools/layerpipe/results/residual_stall
...
nohup /home/sdu/.conda/envs/sllm-worker/bin/python \
  ...
  --output-dir tools/layerpipe/results/residual_stall \
  > tools/layerpipe/results/residual_stall/run.log 2>&1 &
echo $!
```
...
nohup env \
    ...
    TRACE_TIME_SCALE=150 \
    bash tools/layerpipe/run_layerpipe_real_gpu.sh \
    > tools/layerpipe/results/single_gpu/1.2-vmm-odkv-reuse.log 2>&1 &
...
nohup env \
  ...
  TRACE_TIME_SCALE=150 \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh \
  > tools/layerpipe/results/single_gpu/1.2-layerweave.log 2>&1 &
echo $!
```
...
for run in 1 2 3; do
  for prefetch in 0 1; do
    env \
      ...
      OUTPUT="tools/layerpipe/results/multi_gpu/m3-100-prefetch${prefetch}-run${run}.json" \
      bash tools/layerpipe/run_layerpipe_real_gpu.sh \
      > "tools/layerpipe/results/multi_gpu/m3-100-prefetch${prefetch}-run${run}.log" 2>&1
  done
done
```
关键结果：
...
/home/sdu/.conda/envs/sllm-worker/bin/python \
  ...
  --gpu 0 \
  --batch-sizes 1,2,4,8 \
  --output-dir tools/layerpipe/results/m4-full \
  --resume \
  --compact-batches
```
...
/home/sdu/.conda/envs/sllm-worker/bin/python \
  ...
  --output-dir tools/layerpipe/results/m4-m5.5-b1-scale4
...
env GPU_ID=0 \
  ...
  LAYERWEAVE_PREFIX_LAYERS=full \
  OUTPUT=tools/layerpipe/results/multi_gpu/m5.5-marginal-minimal-100.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh \
  > tools/layerpipe/results/multi_gpu/m5.5-marginal-minimal-100.log 2>&1
...
env GPU_ID=0 \
  ...
  LAYERWEAVE_M4_PROFILE=tools/layerpipe/results/m4-m5.5-b1-scale4/m4-full-estimator-report.json \
  LAYERWEAVE_DEMAND_DECAY=0.9 \
  LAYERWEAVE_UNCERTAINTY_MS=100 \
  OUTPUT=tools/layerpipe/results/multi_gpu/m5.5-marginal-dynamic-100.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh \
  > tools/layerpipe/results/multi_gpu/m5.5-marginal-dynamic-100.log 2>&1
```
...
nohup env \
  ...
  TRACE_TIME_SCALE=4 \
  bash tools/layerpipe/run_layerweave_multi_gpu_ab.sh \
  > tools/layerpipe/results/archive/multi_gpu/layerweave-2gpu-globalq1-100-driver.log 2>&1 &
echo $!
```
...
nohup env \
  ...
  bash tools/layerpipe/run_layerweave_multi_gpu_ab.sh \
  > tools/layerpipe/results/archive/multi_gpu/layerweave-2gpu-serial-choice-100-r1-driver.log 2>&1 &
echo $!
```
...
for prefix in 0 2 4 8 16 full; do
  env GPU_ID=0 \
    ...
    OUTPUT="tools/layerpipe/results/archive/multi_gpu/m5-prefix${prefix}-smoke.json" \
    bash tools/layerpipe/run_layerpipe_real_gpu.sh \
    > "tools/layerpipe/results/archive/multi_gpu/m5-prefix${prefix}-smoke.log" 2>&1
done
```
...
env GPU_ID=0 \
  ...
  OUTPUT=tools/layerpipe/results/archive/multi_gpu/m34-service-smoke.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh
```
...
env GPU_ID=0 \
  ...
  OUTPUT=tools/layerpipe/results/multi_gpu/m34-cal-p8-f0.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh
```
...
/home/sdu/.conda/envs/sllm-worker/bin/python \
  ...
  --calibration \
    tools/layerpipe/results/multi_gpu/m34-cal-p0-f0.json tools/layerpipe/results/multi_gpu/m34-cal-p8-f0.json \
    tools/layerpipe/results/multi_gpu/m34-cal-pfull-f0.json tools/layerpipe/results/multi_gpu/m34-cal-p0-f1.json \
    tools/layerpipe/results/multi_gpu/m34-cal-p8-f1.json tools/layerpipe/results/multi_gpu/m34-cal-pfull-f1.json \
  --evaluation \
    tools/layerpipe/results/multi_gpu/m34-eval-p4-f0.json tools/layerpipe/results/multi_gpu/m34-eval-p16-f0.json \
    tools/layerpipe/results/multi_gpu/m34-eval-pfull-f0.json tools/layerpipe/results/multi_gpu/m34-eval-p4-f1.json \
    tools/layerpipe/results/multi_gpu/m34-eval-p16-f1.json tools/layerpipe/results/multi_gpu/m34-eval-pfull-f1.json \
  --output tools/layerpipe/results/multi_gpu/m34-service-ttft-estimator-eval.json
```
...
bash -n tools/layerpipe/run_layerpipe_real_gpu.sh
git diff --check
```
...
env GPU_ID=1 \
  ...
  OUTPUT=tools/layerpipe/results/archive/multi_gpu/m5.5-smoke-fixed.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh
```
...
env GPU_ID=1 \
  ...
  LAYERWEAVE_M4_PROFILE=tools/layerpipe/results/m4-full/m4-full-estimator-report.json \
  LAYERWEAVE_DEMAND_DECAY=0.9 LAYERWEAVE_UNCERTAINTY_MS=0 \
  OUTPUT=tools/layerpipe/results/archive/multi_gpu/m5.5-smoke-dynamic.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh
```
...
/home/sdu/.conda/envs/sllm-worker/bin/python \
  tools/layerpipe/validate_layerweave_m5_5.py \
  --fixed tools/layerpipe/results/archive/multi_gpu/m5.5-smoke-fixed.json \
  --dynamic tools/layerpipe/results/archive/multi_gpu/m5.5-smoke-dynamic.json
```
...
env GPU_ID=0 \
  ...
  TRACE_TIME_SCALE=1 LAYERWEAVE_PREFETCH=1 \
  LAYERWEAVE_CACHE_POLICY=fixed LAYERWEAVE_PREFIX_LAYERS=full \
  OUTPUT=tools/layerpipe/results/multi_gpu/m5.5-fixed-100.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh \
  > tools/layerpipe/results/multi_gpu/m5.5-fixed-100.log 2>&1
```
...
env GPU_ID=0 \
  ...
  OUTPUT=tools/layerpipe/results/multi_gpu/m5.5-dynamic-100.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh \
  > tools/layerpipe/results/multi_gpu/m5.5-dynamic-100.log 2>&1
```
...
/home/sdu/.conda/envs/sllm-worker/bin/python \
  tools/layerpipe/validate_layerweave_m5_5.py \
  --fixed tools/layerpipe/results/multi_gpu/m5.5-fixed-100.json \
  --dynamic tools/layerpipe/results/multi_gpu/m5.5-dynamic-100.json \
  --require-speedup
```
...
- `TangramLayerWeaveController.apply_retained_pages()`
  - 只应用调度器选中的最小 victim set，不再请求结束后对所有模型按
    prefix 做全局裁剪；
- `tools/layerpipe/run_layerpipe_real_gpu.sh`
...
bash -n Tangram/tools/layerpipe/run_layerpipe_real_gpu.sh
...
env GPU_ID=0 \
  ...
  LAYERWEAVE_M4_PROFILE=tools/layerpipe/results/m4-full/m4-full-estimator-report.json \
  LAYERWEAVE_DEMAND_DECAY=0.9 LAYERWEAVE_UNCERTAINTY_MS=0 \
  OUTPUT=tools/layerpipe/results/archive/multi_gpu/m5.5-v2-smoke-dynamic.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh

/home/sdu/.conda/envs/sllm-worker/bin/python \
  tools/layerpipe/validate_layerweave_m5_5.py \
  --fixed tools/layerpipe/results/archive/multi_gpu/m5.5-smoke-fixed.json \
  --dynamic tools/layerpipe/results/archive/multi_gpu/m5.5-v2-smoke-dynamic.json
```
...
env GPU_ID=0 \
  ...
  TRACE_TIME_SCALE=1 LAYERWEAVE_PREFETCH=1 \
  LAYERWEAVE_CACHE_POLICY=fixed LAYERWEAVE_PREFIX_LAYERS=full \
  OUTPUT=tools/layerpipe/results/multi_gpu/m5.5-v2-minimal-100.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh \
  > tools/layerpipe/results/multi_gpu/m5.5-v2-minimal-100.log 2>&1
```
...
env GPU_ID=0 \
  ...
  LAYERWEAVE_M4_PROFILE=tools/layerpipe/results/m4-full/m4-full-estimator-report.json \
  LAYERWEAVE_DEMAND_DECAY=0.9 LAYERWEAVE_UNCERTAINTY_MS=100 \
  OUTPUT=tools/layerpipe/results/multi_gpu/m5.5-v2-dynamic-100.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh \
  > tools/layerpipe/results/multi_gpu/m5.5-v2-dynamic-100.log 2>&1
```
...
/home/sdu/.conda/envs/sllm-worker/bin/python \
  tools/layerpipe/validate_layerweave_m5_5.py \
  --fixed tools/layerpipe/results/multi_gpu/m5.5-v2-minimal-100.json \
  --dynamic tools/layerpipe/results/multi_gpu/m5.5-v2-dynamic-100.json \
  --require-speedup
```
...
```text
tools/layerpipe/results/multi_gpu/m5.5-v2-minimal-100.json
tools/layerpipe/results/multi_gpu/m5.5-v2-minimal-100.log
tools/layerpipe/results/multi_gpu/m5.5-v2-dynamic-100.json
tools/layerpipe/results/multi_gpu/m5.5-v2-dynamic-100.log
```
...
```text
tools/layerpipe/results/m4-m5.5-b1-scale4/m4-full-estimator-report.json
```
...
/home/sdu/.conda/envs/sllm-worker/bin/python \
  tools/layerpipe/validate_layerweave_m5_5.py \
  --fixed tools/layerpipe/results/multi_gpu/m5.5-marginal-minimal-100.json \
  --dynamic tools/layerpipe/results/multi_gpu/m5.5-marginal-dynamic-100.json \
  --require-speedup
```
...
env GPU_ID=0 \
  ...
  TRACE_TIME_SCALE=4 LAYERWEAVE_PREFETCH=1 \
  LAYERWEAVE_CACHE_POLICY=fixed LAYERWEAVE_PREFIX_LAYERS=full \
  OUTPUT=tools/layerpipe/results/archive/multi_gpu/m5.5-marginal-minimal-fix-smoke7.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh
```
...
```text
tools/layerpipe/results/m4-full/m4-full-estimator-report.json
```
...
env GPU_ID=0 \
  ...
  LAYERWEAVE_M4_PROFILE=tools/layerpipe/results/m4-m5.5-b1-scale4/m4-full-estimator-report.json \
  LAYERWEAVE_DEMAND_DECAY=0.9 LAYERWEAVE_UNCERTAINTY_MS=100 \
  OUTPUT=tools/layerpipe/results/archive/multi_gpu/joint-mckp-smoke4.json \
  bash tools/layerpipe/run_layerpipe_real_gpu.sh
```
...
LAYERPIPE_RESULT             = tools/layerpipe/results/archive/multi_gpu/joint-mckp-smoke4.json
```
...
`tools/layerpipe/run_layerweave_joint_ab_2gpu.sh` 在两张独立 GPU 上同时重放完全相同
...
env GPU_A=0 GPU_B=1 MAX_REQUESTS=100 SWAP_GPUS=1 \
  bash tools/layerpipe/run_layerweave_joint_ab_2gpu.sh
```
...
```text
round 1:
  tools/layerpipe/results/multi_gpu/joint-ab-r1-minimal-gpu0.json
  tools/layerpipe/results/multi_gpu/joint-ab-r1-joint-gpu1.json
  tools/layerpipe/results/multi_gpu/joint-ab-r1-validation.txt
round 2 (swap):
  tools/layerpipe/results/multi_gpu/joint-ab-r2-minimal-gpu1.json
  tools/layerpipe/results/multi_gpu/joint-ab-r2-joint-gpu0.json
  tools/layerpipe/results/multi_gpu/joint-ab-r2-validation.txt
```
...
检查发现两侧实际都在物理 GPU-0 分配。根因是 `tools/layerpipe/run_layerpipe_real_gpu.sh`
...
nohup env \
  ...
  bash tools/layerpipe/run_layerweave_multi_gpu_ab.sh \
  > tools/layerpipe/results/multi_gpu/layerweave-2gpu-serial-choice-final-100-r1-driver.log 2>&1 &
echo $!
```
...
```text
tools/layerpipe/results/multi_gpu/layerweave-minimal-2gpu-<tag>.json
tools/layerpipe/results/multi_gpu/layerweave-joint-2gpu-<tag>.json
tools/layerpipe/results/multi_gpu/layerweave-2gpu-<tag>-validation.txt
```
...
nohup env \
  ...
  bash tools/layerpipe/run_layerweave_multi_gpu_ab.sh \
  > tools/layerpipe/results/multi_gpu/layerweave-2gpu-online-final-100-r3-driver.log 2>&1 &
echo $!
```
