...
class GPUMemoryRegion_V4 : public std::enable_shared_from_this<GPUMemoryRegion_V4> {
public:
    ...
    std::shared_ptr<GPUMemoryRegion_V4> prev;
    std::shared_ptr<GPUMemoryRegion_V4> next;

    GPUMemoryRegion_V4() 
        : status(FREE), addr(nullptr), size(0), prev(nullptr), next(nullptr) {}

    GPUMemoryRegion_V4(void* addr_, size_t size_)
        : status(FREE), addr(static_cast<char*>(addr_)), size(size_), 
          prev(nullptr), next(nullptr) {}

    bool isAdjacent(const std::shared_ptr<GPUMemoryRegion_V4>& other) const {
        return (this->addr + this->size == other->addr) || 
               (other->addr + other->size == this->addr);
    }

    void merge(const std::shared_ptr<GPUMemoryRegion_V4>& other) {
    ...
    std::string toString() {
        return "[GPUMemoryRegion_V4: addr=" +
...
struct DropCostEntry_V4 {
    double cost;
    std::shared_ptr<GPUMemoryRegion_V4> region;
    bool operator<(const DropCostEntry_V4& other) const {
...
struct TGNeedAllocates_V4{
    int tg_id;
    TensorGroupIndex tg_index;
    TGNeedAllocates_V4(int tg_id_, TensorGroupIndex tg_index_)
        : tg_id(tg_id_), tg_index(tg_index_) {}
};

class GPUTensorPool_V4 {
public:
    std::shared_ptr<GPUMemoryRegion_V4> memory_regions;  // 内存区域链表头
    std::unordered_map<std::string, std::shared_ptr<GPUMemoryRegion_V4>> allocated_regions;
    ...
    GPUTensorPool_V4(int device_id_, size_t total_size, double gpu_bw, double cpu_bw)
        : device_id(device_id_), GPUBandwidth(gpu_bw), CPUBandwidth(cpu_bw) {
        cudaSetDevice(device_id_);
        void* gpu_memory;
        cudaMalloc(&gpu_memory, total_size);
        cudaStreamCreate(&stream_);
        memory_regions = std::make_shared<GPUMemoryRegion_V4>(gpu_memory, total_size);
        // LOG(INFO)<<"GPUTensorPool_V4: device_id="<<device_id_<<", total_size="<<total_size<<", gpu_bw="<<gpu_bw<<", cpu_bw="<<cpu_bw;
    }
    ...
    ~GPUTensorPool_V4(){
        ...
        // }
        // LOG(INFO)<<"GPUTensorPool_V4: device_id="<<device_id<<", total_move_data="<<total_move_data;
    }
    ...
    // 获取已分配的张量区域
    std::shared_ptr<GPUMemoryRegion_V4> GetTensor(const std::string& fingerprint) {
    ...
    size_t GetMergeCost(){
        ...
        // 找到第一个空闲空间和最后一个空闲空间
        std::shared_ptr<GPUMemoryRegion_V4> first_free = nullptr;
        std::shared_ptr<GPUMemoryRegion_V4> last_free = nullptr;
        while (current) {
    ...
    int GreedyDrop(size_t n, std::string skip_model) {
      ...
      std::set<DropCostEntry_V4> drop_costs;
      ...
      for (auto it = drop_costs.begin();
        ...
        // LOG(INFO)<<"GreedyDrop: release region="<<region->toString()<<"
        // cost="<<it->cost <<" released="<<released<<"
    ...
    void NPMWeightFunc4(
        ...
        std::vector<std::shared_ptr<GPUMemoryRegion_V4>>& free_regions) {
      size_t min_request =
    ...
    std::vector<std::pair<size_t, std::shared_ptr<GPUMemoryRegion_V4>>>
    ...
    //   }
      ...
      std::vector<std::pair<size_t, std::shared_ptr<GPUMemoryRegion_V4>>> matches;
...
// 合并后的空闲区域指针，如果合并失败则返回nullptr
    std::shared_ptr<GPUMemoryRegion_V4> MergeRegions(
        std::shared_ptr<GPUMemoryRegion_V4> region_start,
        std::shared_ptr<GPUMemoryRegion_V4> region_end, bool is_do_move = 1) {
        size_t move_cost=0;
        // 检查输入的起始和结束区域指针是否有效
        ...
        std::vector<std::shared_ptr<GPUMemoryRegion_V4>> allocated_regions_tmp;
        ...
        // 创建新空闲区域
        std::shared_ptr<GPUMemoryRegion_V4> free_region =
            std::make_shared<GPUMemoryRegion_V4>(current_addr, free_region_size);
    ...
    // 贪心合并策略
    std::shared_ptr<GPUMemoryRegion_V4> GreedyMerge(size_t n, bool merge=1) {
        // 先检查是否有足够大的空闲区域
        auto current = memory_regions;
        size_t total_free = 0;
        while (current) {
        ...
        std::shared_ptr<GPUMemoryRegion_V4> best_s, best_e;
    ...
    void FreeRegion(std::shared_ptr<GPUMemoryRegion_V4> region, bool debug=0) {
        if(!region){
    ...
    std::shared_ptr<GPUMemoryRegion_V4> AllocateFreeRegion(
        std::shared_ptr<GPUMemoryRegion_V4> free_region, size_t allocate_size, 
        const std::string& fingerprint) {
        if (free_region->status != FREE || free_region->size < allocate_size) return nullptr;
        if (free_region->size > allocate_size) {
            auto new_region = std::make_shared<GPUMemoryRegion_V4>(
...
std::pair<std::shared_ptr<GPUMemoryRegion_V4>, std::shared_ptr<GPUMemoryRegion_V4>> AllocateNewRegion(
    std::shared_ptr<GPUMemoryRegion_V4> old, size_t allocate_size,
    const std::string& fingerprint) {
  ...
  std::shared_ptr<GPUMemoryRegion_V4> new_region = nullptr;
  if (old->size > allocate_size) {
    new_region = std::make_shared<GPUMemoryRegion_V4>(
...
void CopyMemoryLayout(GPUTensorPool_V4& other) {
    // 清空当前对象的内存区域和已分配区域
    ...
    // 复制GPU内存区域链表
    std::shared_ptr<GPUMemoryRegion_V4> other_current = other.memory_regions;
    std::shared_ptr<GPUMemoryRegion_V4> prev = nullptr;
    std::shared_ptr<GPUMemoryRegion_V4> new_head = nullptr;
  ...
  if (is_regenerate == 0) {
    ...
    if (ifs) {
      ...
      for (int i = 0; i < saved_regions; ++i) {
        auto region = std::make_shared<GPUMemoryRegion_V4>();
...
std::vector<size_t> MockBartiteMatching(std::vector<size_t> requests,
  ...
  return remaining;
    ...
    struct RegionAllocateGroup{
        std::shared_ptr<GPUMemoryRegion_V4> start;
        std::shared_ptr<GPUMemoryRegion_V4> end;
        std::vector<TGNeedAllocates_V4> tg_to_loads;
    ...
    struct CanGroupSplitResult {
      bool success;
      std::vector<TGNeedAllocates_V4> to_left_requests;
      std::vector<TGNeedAllocates_V4> to_right_requests;
    };
    ...
    */
    CanGroupSplitResult CanGroupSplit(size_t left_total_size,
                                      ...
                                      std::vector<TGNeedAllocates_V4> requests) {
    ...
    std::pair<RegionAllocateGroup, RegionAllocateGroup> BestSplitAllocate(
      ...
      std::vector<std::shared_ptr<GPUMemoryRegion_V4>>
    ...
    std::vector<RegionAllocateGroup> RecursiveSplitAllocate(
      ...
      // current = memory_regions;
      std::shared_ptr<GPUMemoryRegion_V4> last = nullptr;
      while (current) {
        if (current->status == FREE) {
          last = current;
        }
    ...
    void MockRecursiveSplitAllocate(std::vector<size_t> requests){
        ...
        std::vector<TGNeedAllocates_V4> tg_to_loads;
        for(size_t request_size : requests) {
            tg_to_loads.push_back(TGNeedAllocates_V4(fp_id, TensorGroupIndex(0, request_size, std::to_string(fp_id),{})));
            fp_id++;
        }
...
class VRAMManager_V4 : public VRAMManagerBase {
private:
    std::unordered_map<std::string, std::shared_ptr<RegisteredModel>> registered_models_;
    std::unordered_map<int, std::shared_ptr<GPUTensorPool_V4>> gpu_tensor_pools_;
    std::mutex mutex_;
...
public:
    VRAMManager_V4(size_t gpu_tensor_pool_size, const std::vector<int>& gpu_ids, 
                  double gpu_bw, double cpu_bw) {
        for (int gpu_id : gpu_ids) {
            gpu_tensor_pools_[gpu_id] = std::make_shared<GPUTensorPool_V4>(
                gpu_id, gpu_tensor_pool_size, gpu_bw, cpu_bw);
        }
    }

    ~VRAMManager_V4() {
      #ifdef RecursiveSplitEnable
        LOG(INFO)<<"RecursiveSplitEnabled";
      #else
        #ifdef BipartMatchEnable
        LOG(INFO)<<"BipartiteMatchingEnabled";
        #endif
        LOG(INFO)<<"GreedyMerge";
      #endif
        // 统计平均模型加载和合并延迟
    ...
    /*
        基于二分匹配的模型加载方法：
            1. 基于贪心模型释放策略，确保有足够的空间加载模型
            2. 基于二分匹配算法，找到所有可以优先分配的TG（比如分配后碎片最小，分配后合并成本最低）
            3. 贪心地合并剩余空间，装载剩余的TG
    */
    std::string LoadModel(const std::string& model_path, int device_id) {
        ...
        std::vector<TGNeedAllocates_V4> tg_to_load;
        ...
        LOG(INFO)<<"LoadModel: model_path="<<model_path<<" to_allocates.size()="<<tg_to_load.size();
        ...
        for (auto group : region_groups) {
          ...
          // 分配TG
          shared_ptr<GPUMemoryRegion_V4> next_free = merged_region;
          shared_ptr<GPUMemoryRegion_V4> allocated = nullptr;
          // LOG(INFO)<<"TG to load: ";
        ...
        std::vector<TGNeedAllocates_V4> remaining = tg_to_load;
#ifdef BipartMatchEnable
        ...
        if (!remaining.empty()) {
            ...
            std::sort(remaining.begin(), remaining.end(),
                      [](const TGNeedAllocates_V4& a, const TGNeedAllocates_V4& b) {
                        return a.tg_index.size > b.tg_index.size;
                      });
        ...
        // for(auto id: tg_to_load_ids){
        //   if(!allocated_regions[id]){
        //     LOG(ERROR)<<"allocated_regions["<<id<<"] is nullptr";
        //     return "ERROR";
        //   }
        // }
