...
MAX_REQUESTS=1000 docs/legacy/reuse_only_tangram/6-end2end-sim.sh
```
...
```bash
cd /mnt/n0/Tangram/Tangram
BACKEND=cpp MAX_REQUESTS=1000 docs/legacy/reuse_only_tangram/6-end2end-sim.sh
```
...
MAX_REQUESTS=1000 \
docs/legacy/reuse_only_tangram/6-end2end-sim.sh
```
...
```bash
cd /mnt/n0/Tangram/Tangram
MAX_REQUESTS=0 docs/legacy/reuse_only_tangram/6-end2end-sim.sh
```
...
```bash
RPS_LIST="0.4 1.6" GPU_LIST="1 4 8" MAX_REQUESTS=1000 docs/legacy/reuse_only_tangram/6-end2end-sim.sh
```

不重新生成 trace，只复用已有 trace：

```bash
GENERATE_TRACES=0 MAX_REQUESTS=1000 docs/legacy/reuse_only_tangram/6-end2end-sim.sh
```

如果 trace 不存在，先生成 trace：

```bash
GENERATE_TRACES=1 MAX_REQUESTS=1000 docs/legacy/reuse_only_tangram/6-end2end-sim.sh
```

不画图，只生成数据：

```bash
PLOT=0 MAX_REQUESTS=1000 docs/legacy/reuse_only_tangram/6-end2end-sim.sh
```

指定 SLLM-CM keep alive：

```bash
SLLM_KEEP_ALIVE_MS=60000 MAX_REQUESTS=1000 docs/legacy/reuse_only_tangram/6-end2end-sim.sh
```
...
```bash
LOAD_BANDWIDTH_GBPS=24 MAX_REQUESTS=1000 docs/legacy/reuse_only_tangram/6-end2end-sim.sh
```
...
```bash
CONFIG_PATH=/path/to/your_config.json MAX_REQUESTS=1000 docs/legacy/reuse_only_tangram/6-end2end-sim.sh
```

如果 trace 已经提前生成，也可以指定：

```bash
TRACE_BASE=/path/to/servegen_tangram_end2end.trace GENERATE_TRACES=0 docs/legacy/reuse_only_tangram/6-end2end-sim.sh
```
