...
minimal_output="docs/layerweave-minimal-2gpu-${RUN_TAG}.json"
joint_output="docs/layerweave-joint-2gpu-${RUN_TAG}.json"
...
"$PYTHON" tools/layerpipe/validate_layerweave_multi_gpu_ab.py \
  ...
  --joint "$joint_output" |
  tee "docs/layerweave-2gpu-${RUN_TAG}-validation.txt"
